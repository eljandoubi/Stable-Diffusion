...
parser.add_argument(
    "--wandb_run_name", required=True, type=str, metavar="wandb_run_name"
)
...
parser.add_argument(
    "--path_to_dataset", help="Root directory of dataset", required=True, type=str
)
...
with open(args.training_config, "r") as f:
    training_config: dict[str, Any] = yaml.safe_load(f)["training_args"]

with open(args.model_config, "r") as f:
    vae_config: dict[str, Any] = yaml.safe_load(f)["vae"]
...
if args.log_wandb:
    accelerator.init_trackers(
        args.experiment_name, init_kwargs={"wandb": {"name": args.wandb_run_name}}
    )
...
accelerator.print(
    f"LATENT SPACE DIMENSIONS: {config.latent_channels, latent_res, latent_res}"
)
...
if training_config["use_patchgan"]:
    ...
    discriminator = PatchGAN(
        ...
        leaky_relu_slope=training_config["disc_leaky_relu"],
    ).apply(init_weights)
...
### Get DataLoader ###
mini_batchsize = (
    training_config["per_gpu_batch_size"]
    // training_config["gradient_accumulations_steps"]
)
...
effective_epochs = (
    ...
    * training_config["total_training_iterations"]
) / len(dataset)
...
if training_config["val_img_folder_path"] is not None:
    val_images = load_val_images(
        path_to_image_folder=training_config["val_img_folder_path"],
        img_size=vae_config["img_size"],
...
def reset_log(log: dict[str, float]):
    return {key: 0.0 for key in log.keys()}
...
progress_bar = tqdm(
    range(training_config["total_training_iterations"]),
    initial=global_step,
    ...
    for i, batch in enumerate(dataloader):
        ...
        if generator_step:
            ...
            with accelerator.accumulate(model):
                ...
                if training_config["reconstruction_loss_fn"] == "l1":
                    ...
                    raise ValueError(
                        f"{training_config['reconstruction_loss_fn']} is not a Valid Reconstruction Loss"
                    )
                ...
                ### Add Together Losses ###
                perceptual_loss = (
                    reconstruction_loss + training_config["lpips_weight"] * lpips_loss
                )
                ...
                if train_disc:
                    ...
                    norm_grad_wrt_gen_loss = (
                        torch.autograd.grad(
                            outputs=gen_loss, inputs=last_layer, retain_graph=True
                        )[0]
                        .detach()
                        .norm(p=2)
                    )
                    ...
                    loss = (
                        loss
                        + adaptive_weight * gen_loss * training_config["disc_weight"]
                    )
                ...
                for key, value in log.items():
                    model_log[key] += (
                        value.mean() / training_config["gradient_accumulations_steps"]
                    )
        ...
        if accelerator.sync_gradients:
            ...
            if model_toggle or not train_disc:
                ## Gather Across GPUs ###
                model_log = {
                    key: accelerator.gather_for_metrics(value).mean().item()
        ...
        if (global_step % training_config["checkpoint_iterations"] == 0) or (
            ...
            path_to_checkpoint = os.path.join(
                path_to_experiment, f"checkpoint_{global_step}"
            )

...
def conceptual_captions(path_to_data:str, transforms:Callable[[Image.Image],torch.Tensor]):
...
def ConceptualCaptionsCollateFunction(model_name:str="openai/clip-vit-large-patch14",
    ...
    def _collate_fn(batch):
        ...
        if return_captions:
            if pre_encoded_text:
                ...
                output = tokenizer([b["caption"] for b in batch],return_tensors= "pt",padding=True)
...
def get_dataset(dataset:Literal["celebahq","imagenet","conceptual_captions"],
                path_to_data:str, 
                num_channels:int=3, 
                img_size:int=256, 
                random_resize:bool=True, 

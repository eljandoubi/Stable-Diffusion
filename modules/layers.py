...
class ResidualBlock2D(nn.Module):
    ...
    def __init__(
        ...
        time_embed_dim: Optional[int] = 1280,
        class_embed_proj=False,
        class_embed_dim: Optional[int] = 512,
        norm_eps: float = 1e-6,
    ):
    ...
    def forward(
        ...
        if time_embed is not None:
            if self.time_expand is None:
                raise Exception(
                    "Passing in Time Embedding into ResidualBlock without time_embed_proj = True"
                )
            else:
        ...
        if class_conditioning is not None:
            if self.class_expand is None:
                raise Exception(
                    "Passing in Class Conditioning to ResidualBlock without class_embed_proj = True"
                )
            else:

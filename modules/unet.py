...
class UpBlock2D(nn.Module):
    ...
    def forward(
        self,
        x: torch.Tensor,
        residual_block_tuple: list[torch.Tensor],
        time_embed: torch.Tensor,
        text_conditioning: Optional[torch.Tensor] = None,
        text_attention_mask: Optional[torch.Tensor] = None,
        class_conditioning: Optional[torch.Tensor] = None,
...
class UNet2DModel(nn.Module):
    ...
    def __init__(self, config: LDMConfig):
        ...
        for i, block_type in enumerate(config.up_block_types):
            ...
            input_channel = reversed_channels_per_block[
                min(i + 1, len(reversed_channels_per_block) - 1)
            ]
        ...
        self.conv_out = nn.Conv2d(
            config.unet_channels_per_block[0],
    ...
    def forward(
        ...
        x = self.mid_block(
            x, time_embed, text_conditioning, text_attention_mask, class_conditioning
        )

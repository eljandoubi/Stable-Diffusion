...
def count_num_params(model: Module):
...
def load_val_images(
    path_to_image_folder: str, img_size: int, device: torch.device, dtype: str
):
...
def save_orig_and_generated_images(
    original_images: torch.Tensor,
    generated_image_tensors: torch.Tensor,
    ...
    if not os.path.isdir(path_to_save_folder):
        accelerator.print(
            f"Creating Folder {path_to_save_folder} to save Reconstructions"
        )
    ...
    generated_image_tensors = torch.clamp(generated_image_tensors, -1.0, 1.0)
    ...
    final_image = Image.new(
        mode="RGB", size=(img_width * len(combined_images), 2 * img_height)
    )
    x_offset = 0
...
def load_testing_text_encodings(
    path_to_text: str = "inputs/sample_text_cond_prompts.txt",
...
def load_testing_imagenet_encodings(
    path_to_imagenet_labels: str = "inputs/imagenet_class_prompt.txt",
):
